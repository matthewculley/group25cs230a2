	...
	public void processKeyEvent(KeyEvent event) {
		...
		switch (event.getCode()) {
		    case RIGHT:
		    	...
		    	if (checkValidMove(map.getPlayer().getX() + 1, map.getPlayer().getY())) {
...
//		    	System.out.println("\n" + map.getAt(map.getPlayer().getX(), map.getPlayer().getY()).toString());
		    ...
		    case UP:
		    	// Right key was pressed. So move the player right by one cell.
		    	if (checkValidMove(map.getPlayer().getX(), map.getPlayer().getY() - 1)) {
		...
		for (int k = 0; k < map.getCollectibles().size(); k++) {
			if (map.getCollectibles().get(k).getX() == map.getPlayer().getX() & map.getCollectibles().get(k).getY() == map.getPlayer().getY() & map.getCollectibles().get(k).isCollected() == false) {
				...
				//add to inventory
				map.getPlayer().getInventory().addItem(map.getCollectibles().get(k));
	...
	private boolean checkValidMove(int x, int y) {
		System.out.println("Check if cell (" + x + "," + y + ") is a valid move");
...
//		
//		else if (!(map.getAt(x, y).isPassable())) {
//			System.out.println("Cant move to: " + map.getAt(x, y).toString() + map.getAt(x, y).isPassable());
//			return false;
//			if (map.getPlayer().getInventory().hasItem(new Flippers())) {
//				return true;
//			}
//			return false;
//		} else {
...
//	}
	...
	public void drawGame() {
		...
		int newZeroX = map.getPlayer().getX() - offset;
		int newZeroY = map.getPlayer().getY() - offset;
...
//				gc.drawImage(new Image("enemyDumb.png"), newZeroX * GRID_CELL_WIDTH, newZeroY * GRID_CELL_HEIGHT);
...
//		map.getPlayer().getX() = 0;
//		map.getPlayer().getY() = 0;
//		drawGame();
//	}
//	
//	/**
...
//		map.getPlayer().getX() = 2;
//		map.getPlayer().getY() = 2;
//		drawGame();		
//	}
